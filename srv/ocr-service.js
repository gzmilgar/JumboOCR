...
this.on('UPDATE', 'OCRItems', async (req) => {
    ...
    this.on('extractAndProcess', async (req) => {
        ...
        try {
            ...
            var schemaName = req.data.schemaName || '';
            console.log('=== extractAndProcess called [' + processName + '] templateName=' + templateName + ' schemaName=' + schemaName + ' ===');
            ...
            var extractedData = await extractDocumentWithDox(pdfBase64, processName, pdfName, templateName, schemaName);
...
async function s4Patch(entityWithKey, body) {
    ...
    async function extractDocumentWithDox(pdfBase64, processName, fileName, templateName, schemaName) {
        ...
        // Template config from BPA parameters
        var templateConfig = {
            schemaName: schemaName || DOX_SCHEMA_NAME,
