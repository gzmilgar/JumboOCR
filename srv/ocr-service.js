...
module.exports = class extends cds.ApplicationService {
    async init() {
        ...
        async function _executeSalesOrderCreation(uuid, processName) {
            ...
            var minData = {
                ...
                lineItems: logEntry.items.map(function (i) {
                    return {
                        barcode: i.Barcode || '', quantity: String(i.Quantity || ''), unitPrice: String(i.UnitPrice || ''),
        ...
        async function _processSalesOrder(data, stsa, processName) {
            ...
            var barcodes = extracted.barcodes;
            var taxId = extracted.taxId;
            console.log('[' + processName + '] Barcodes:' + barcodes.length + ' TaxId:' + taxId);

            var eanProductMap = await lookupProducts(barcodes);
            console.log('[' + processName + '] Products:' + Object.keys(eanProductMap).length + '/' + barcodes.length);

            var barcodeReport = checkBarcodes(barcodes, eanProductMap);
        ...
        async function autoSavePOLog(fields) {
            ...
            try {
                ...
                // --- SAP İÇİN TARİH FORMATLAYICI ---
                // Uzun tarih metinlerini (Wed Dec 17...) SAP'nin beklediği (2025-12-17) formatına çevirir
                function formatSapDate(dateVal) {
                    if (!dateVal) return null;
                    var d = new Date(dateVal);
                    if (!isNaN(d.getTime())) {
                        return d.toISOString().split('T')[0]; // "YYYY-MM-DD" parçasını alır
                    }
                    return null;
                }
                ...
                var body = {
                    ...
                    // TARİHLER BURADA TEMİZLENİP SAP'YE GÖNDERİLİYOR
                    DeliveryDate: formatSapDate(fields.deliveryDate),
                    DocumentDate: formatSapDate(fields.documentDate),
                ...
                if (e.response && e.response.data) {
                    console.error('S/4HANA POST 400 Detayı: ' + JSON.stringify(e.response.data));
                } else if (e.rootCause) {
                    console.error('Root Cause: ' + e.rootCause.message);
                }
        ...
        function extractEansAndTaxId(data) {
            ...
            var taxId = getField(data.headerFields, 'taxId')
            ...
            return { barcodes: barcodes, taxId: taxId };
        }
        ...
        // LOOKUP PRODUCTS
        // ============================================================
        async function lookupProducts(barcodes) {
            if (!barcodes || barcodes.length === 0) return {};
            var productMap = {};
            var BATCH = 50;

            // Step 1: Try all barcodes as EAN (ProductStandardID)
            for (var i = 0; i < barcodes.length; i += BATCH) {
                var batch = barcodes.slice(i, i + BATCH);
                var filterParts = batch.map(function (bc) {
                    return "ProductStandardID eq '" + String(bc) + "'";
                });
            ...
            // Step 2: For barcodes not found by EAN, try Description (MAKTX) lookup
            var missing = barcodes.filter(function (bc) { return !productMap[bc]; });
            if (missing.length > 0) {
                console.log('lookupProducts: EAN not found, trying description for: ' + missing.join(', '));
                for (var d = 0; d < missing.length; d++) {
                    var bc = missing[d];
                    try {
                        var descUrl = "/sap/opu/odata/sap/API_PRODUCT_SRV/A_ProductDescription"
                            + "?$filter=" + encodeURIComponent("substringof('" + bc + "',ProductDescription)")
                        ...
                        if (descResults.length > 0) {
                            ...
                            console.log('lookupProducts: Description match "' + bc + '" → Material=' + foundProduct);
                            ...
                            console.log('lookupProducts: No description match for "' + bc + '"');
                        }
                    } catch (e) {
                        console.error('Product description lookup error for "' + bc + '": ' + e.message);
                    }
                }
            }
        ...
        async function buildPayload(data, eanProductMap, ctx) {
            ...
            // =======================================================================
            // SEPHORA VE EMAX ÖZEL: TESLİMAT TARİHİ KONTROLÜ
            // =======================================================================
            var currentBrand = (processName || "").toLowerCase();
            // Marka isminde sephora VEYA emax (örn: emaxhtml) geçiyorsa bu kural çalışır
            var isDateFallbackNeeded = (currentBrand.indexOf("sephora") >= 0 || currentBrand.indexOf("emax") >= 0);

            if (isDateFallbackNeeded) {
                // 1. Eğer Header'da tarih yoksa, Line Item'ların ilkine bak
                if (!deliveryDate && data.lineItemFields && data.lineItemFields.length > 0) {
                    // BPA (Excel) veya DOX'tan gelen olası alan isimlerini kontrol et
                    deliveryDate = getField(data.lineItemFields[0], 'deliveryDate') ||
                        getField(data.lineItemFields[0], 'Plan.del.') || "";
                    console.log('buildPayload (' + processName + '): Header deliveryDate boş, lineItem[0]\'dan alındı: ' + deliveryDate);
                }
            }

            var sapDeliveryDate = toSapDate(deliveryDate);

            // 2. Eğer hala geçerli bir tarih yoksa, SAP hata fırlatmasın diye bugünü atıyoruz
            if (isDateFallbackNeeded && !sapDeliveryDate) {
                sapDeliveryDate = toSapDate(new Date().toISOString());
                console.log('buildPayload (' + processName + '): Geçerli tarih bulunamadı, bugünün tarihi (fallback) atandı: ' + sapDeliveryDate);
            }
            // =======================================================================
            ...
            var shipToResult = findShipTo(receiverId, hdr, shipToList, processName);
            if (shipToResult.shipToId) {
                shipToResult.shipToId = String(shipToResult.shipToId).replace(/^0+/, '');
            }
            if (shipToResult.soldToParty) {
                shipToResult.soldToParty = String(shipToResult.soldToParty).replace(/^0+/, '');
            }
            ...
            var soldToParty = shipToResult.soldToParty ? shipToResult.soldToParty.replace(/^0+/, '') : '';
            var companyCode = '';
            ...
            var partnerArray = [];
            if (shipToResult.shipToId) {
                var cleanShipToId = shipToResult.shipToId.replace(/^0+/, '');
                partnerArray.push({ PartnerFunction: 'WE', Customer: cleanShipToId });
            }
        ...
        // FIND SHIP TO
        // ============================================================
        function findShipTo(receiverId, headerFields, shipToList, processName) {
            ...
            // =========================================================================
            // SADECE SEPHORA İÇİN ÖZEL EŞLEŞTİRME MANTIĞI
            // =========================================================================
            var currentBrand = (processName || "").toLowerCase();
            if (currentBrand === "sephora") {
                console.log('findShipTo: Sephora özel eşleştirme mantığı çalıştırılıyor...');
                var recId = (receiverId || "").trim().toUpperCase();
                var excelAddrClean = (deliveryAddress || "").toUpperCase().replace(/[^A-Z0-9]/g, '');

                for (var i = 0; i < shipToList.length; i++) {
                    var sapRecord = shipToList[i];
                    var sapAddress = String(sapRecord.ShipToAddress || '').toUpperCase();
                    var sapAddressClean = sapAddress.replace(/[^A-Z0-9]/g, '');

                    // Excel'deki ID ("7502") SAP adresinin içindeyse VEYA 
                    // temizlenmiş adresler ("SEPHORADUBAIMALL") birbiriyle eşleşiyorsa
                    if ((recId && sapAddress.indexOf(recId) >= 0) ||
                        (excelAddrClean && excelAddrClean.length > 5 && sapAddressClean.indexOf(excelAddrClean) >= 0)) {

                        var sephoraResult = {
                            shipToId: String(sapRecord.ShipToId || ''),
                            soldToParty: String(sapRecord.SoldToParty || '').trim(),
                            company: String(sapRecord.Company || '').trim(),
                            sapCompanyCode: String(sapRecord.SapCompanyCode || '').trim()
                        };
                        console.log('findShipTo: Sephora match → ShipToId=' + sephoraResult.shipToId +
                            ' SoldToParty=' + sephoraResult.soldToParty +
                            ' SapCompanyCode=' + sephoraResult.sapCompanyCode +
                            ' (addr: ' + sapAddress + ')');
                        return sephoraResult;
                    }
                }
                console.log('findShipTo: Sephora özel eşleşmesi bulunamadı, standart arama deneniyor.');
            }
            // =========================================================================


            // =========================================================================
            // DİĞER MARKALAR (VE SEPHORA BULUNAMAZSA) İÇİN ORİJİNAL MANTIK
            // =========================================================================
            if (receiverId) {
            ...
            // Fuzzy (Kelime Bazlı Yüzdesel) Eşleşme Kontrolü
                    ...
                    if (score > best.score && score >= 0.5) {
                        best = {
                            result: {
                                ...
                                sapCompanyCode: String(row3.SapCompanyCode || '').trim()
                            }, score: score
                        };
                    }
                }
        ...
        function toSapDate(dateStr) {
            ...
            // YENİ EKLENEN: BPA'dan gelen "Date(2025-12-17T00:00:00.000Z)" formatını temizle
            var bpaDate = normalized.match(/^Date\((.*?)\)$/i);
            if (bpaDate) {
                normalized = bpaDate[1];
            }
